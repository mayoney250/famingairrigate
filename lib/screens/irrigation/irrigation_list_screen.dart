...
class _IrrigationListScreenState extends State<IrrigationListScreen> {
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    ListTile(
                      ...
                      subtitle:
                          Text(DateFormat('yMMMd hh:mm a').format(start)),
  ...
  void _deleteSchedule(
      BuildContext context, IrrigationScheduleModel schedule) async {
                ...
                Expanded(
                  child: ElevatedButton(
                    onPressed: () async {
                      try {
                        final ok =
                            await _irrigationService.deleteSchedule(schedule.id);
                        Get.back();
                        if (ok) {
  ...
  Future<void> _toggleSchedule(BuildContext context,
      IrrigationScheduleModel schedule, String userId) async {
    final scheme = Theme.of(context).colorScheme;
    bool isOn = schedule.status == 'running';
    final ok = await _irrigationService.toggleSchedule(schedule.id, isOn);
    if (ok) {
      Get.snackbar(isOn ? 'Stopped' : 'Started',
          isOn ? 'Irrigation stopped' : 'Irrigation started',
...
class _ScheduleCardState extends State<_ScheduleCard>
  ...
  void _initStateWithCountdown() {
    ...
    _remaining = isRunning
        ? widget.schedule.startTime
            .add(Duration(minutes: widget.schedule.durationMinutes))
  ...
  void _handleCountdownTick() {
    ...
    final end =
        start.add(Duration(minutes: widget.schedule.durationMinutes));
    bool justBecameRunning = !isRunning &&
        now.isAfter(start) &&
        now.isBefore(end) &&
  ...
  Widget build(BuildContext context) {
    ...
    final endTime =
        schedule.startTime.add(Duration(minutes: schedule.durationMinutes));
    // Calculate seconds left for better mm:ss formatting
    final secondsLeft =
        _remaining != null && _remaining!.inSeconds > 0 ? _remaining!.inSeconds : 0;
  ...
  void _checkScheduleCompletion(BuildContext context) {
    final now = DateTime.now();
    final end = widget.schedule.startTime.add(Duration(minutes: widget.schedule.durationMinutes));
    final justBecameRunning =
        now.isAfter(widget.schedule.startTime) && !isRunning && widget.schedule.status != 'completed';
    if (justBecameRunning) {
      setState(() {
        isRunning = true;
      });
